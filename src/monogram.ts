...
export function monogramSvg(source: MonogramSource) {
  ...
  const primary = domainColor ? `hsl(${hue} 72% 58%)` : safeColor(style.primary, "#4F7CFF");
  const secondary = domainColor ? `hsl(${(hue + 28) % 360} 68% 42%)` : safeColor(style.secondary, "#745CFF");
