...
- The required summary contains the session sample counts, Incremental interaction P95, slowest Full discovery, and slowest Rule freshness interval. It may include stage-level totals or maxima when they make a hotspot actionable, but it must remain a concise single-session summary. An Incremental interaction is one render batch with local discovery work; batches that only rebuild rules or publish rules without discovery are not interactions. Interaction counts are session totals, while the P95 may be computed from a bounded retained window of the session's samples.
